...
$app->post('/api/messages/{id:[0-9]+}/{id:[0-9]+}', function ($author_id, $recipient_id) use ($app) {
    $message = $app->request->getJsonRawBody();
    $response = new Response();
    $content = ['code' => 404, 'status' => 'Not Found', 'data' => []];
    if(!isset($message->text) || empty($message->text) || strlen($message->text))
    {
        $content = ['code' => 500, 'status' => 'Internal Error', 'data' => 
            ['Invalid message text, it is not set or empty or longer than expected.']];
    }
    
    $author = User::findFirst(['id' => $author_id]);
    if(empty($author))
    {
        $content['data'][] = 'There is no user with such author id.';
    }
    
    $recipient = User::findFirst(['id' => $recipient_id]);
    if(empty($recipient))
    {
...
$app->post('/api/friends/{id:[0-9]+}/{id:[0-9]+}', function ($user_id, $friend_id) use ($app) {
    $response = new Response();
    $content = ['code' => 404, 'status' => 'Not Found', 'data' => []];
    ...
    $friend = User::findFirst(['id' => $friend_id]);
    if(empty($friend))
    {
    ...
    if(empty($content['data']))
    {
        $friends = UsersFriends::findFirst(['user_id' => $friend_id, 'friend_id' => $user_id]);
